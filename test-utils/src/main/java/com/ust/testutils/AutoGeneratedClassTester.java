 ...
 * @author rob.dawson
 */
public class AutoGeneratedClassTester {
    ...
    public static void testEntity(final Class<?> clazz) throws Exception {
        ...
        for (Class<?> eventClass : eventClasses) {
            List<String> eventFields = Arrays.stream(eventClass.getDeclaredFields()).map(Field::getName).collect(Collectors.toList());
            Event evt = (Event)eventClass.getDeclaredConstructors()[0].newInstance(eventFields.stream().map(values::get).toArray());
    ...
    public static void testEntityItem(final Class<?> clazz) throws Exception {
        ...
        for (Map.Entry<Class<?>, Method> eventClass : eventClasses.entrySet()) {
            List<String> eventFields = Arrays.stream(eventClass.getKey().getDeclaredFields()).map(Field::getName).collect(Collectors.toList());
            Event evt = (Event)eventClass.getKey().getDeclaredConstructors()[0].newInstance(eventFields.stream().map(values::get).toArray());
    ...
    private static Object getGetterValue(Field field, Object object) {
        try {
            Object invoke = object.getClass().getDeclaredMethod(getGetter(field)).invoke(object);
            return invoke;
        } catch (IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
